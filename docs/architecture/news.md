...
기술 아티클 사이트에서 전날(00:00~24:00, KST) 게시된 글 목록을 스크래핑하여 Google Calendar에 종일 이벤트로 등록한다.
...
- 필터: `src/shared/date.ts` 기준 전날 날짜(KST)와 일치하는 항목만 수집
...
- 필터: 게시일을 `YYYY-MM-DD`로 정규화한 뒤 `src/shared/date.ts` 기준 전날 날짜(KST)와 일치하는 항목만 수집
...
- 방식: Playwright 페이지 대신 `fetch()`로 RSS XML을 가져온 뒤, 전날 날짜(KST)와 일치하는 `Issue #...` / `Newsletter` 아이템 1건을 선택
...
### `src/shared/date.ts`
서울 기준 오늘/전날/다음 날짜를 `YYYY-MM-DD` 형식으로 계산하는 공통 유틸.
