...
│   ├── calendar.ts           # 카테고리별 공지 설명 생성 + 공통 캘린더 업로드 호출
│   └── scraper/
│       ├── index.ts          # 카테고리·소스 레지스트리
│       ├── nowon.ts          # 노원구 공지사항
│       ├── yp21.ts           # 양평군 공통 BBS
│       └── seoulMediahub.ts  # 서울 미디어허브
...
각 기능(`vocab`, `tech-news`, `district-notice`, `todo`)은 독립적인 진입점을 가진다.
...
| district-notice | `{eventKey}{YYYYMMDD}` | `noron20260417`, `angpeng20260417`, `seoul20260417` |
...
- `daily-district-notice.yml`: district notice 전용
- `daily-todo.yml`: todo 전용
- vocab 실행 시각: 매일 **05:30 KST** (전일 20:30 UTC)
- tech-news 실행 시각: 매일 **09:10 KST** (00:10 UTC), **18:07 KST** (09:07 UTC), **23:13 KST** (14:13 UTC)
- district notice 실행 시각: 매일 **18:00 KST** (09:00 UTC)
...
| 변수 | 필수 | 설명 |
|------|------|------|
| `GOOGLE_SERVICE_ACCOUNT_KEY_JSON` | vocab/tech-news/district-notice/todo 공통 | 서비스 계정 키 JSON 문자열 (GitHub Actions용) |
| `GOOGLE_SERVICE_ACCOUNT_KEY_PATH` | 로컬 개발용 | 서비스 계정 키 파일 경로 |
| `CALENDAR_ID` | vocab/district-notice 공통 | Google Calendar ID |
| `ATTENDEE_EMAIL` | 선택 | 이벤트 초대 이메일 |
| `DISTRICT_NOTICE_TARGET_DATE` | district-notice 선택 | 재수집/재업로드할 기준 날짜 (`YYYY-MM-DD`) |
