...
날짜 속성이 비어 있으면 제외한다.
Status가 `"할 일"`인 항목은 캘린더에 업서트하기 직전에 `"진행 중"`으로 변경한다.
...
src/todo/index.ts
  ...
  └─ upsertTodoEvents()      # 항목별 Google Calendar 업서트
       ├─ Status == "할 일" → Notion Status를 "진행 중"으로 변경
...
- `upsertTodoEvents()`: 각 항목을 순회하며, Status가 `"할 일"`이면 먼저 Notion에서 `"진행 중"`으로 변경한 뒤 `calendar.ts`의 upsert 함수 호출
...
- Notion Date 응답 구조 예시:
  ...
  ```
- Status가 `"할 일"`이면 캘린더 업서트 전에 Notion 페이지를 `"진행 중"`으로 먼저 갱신
